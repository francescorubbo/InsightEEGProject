...
def reformatInput(data, labels, indices, trainfrac):
    """
    Receives the the indices for train and test datasets.
    Outputs the train, validation, and test data and label datasets.
    """
    print('len(indices[0])',len(indices[0]))
    ntrainfrac = int(len(indices[0])*trainfrac)
    print('ntrainfrac',ntrainfrac)
    fracIndices = indices[0][:ntrainfrac]
    trainIndices = fracIndices[len(indices[1]):]
    validIndices = fracIndices[:len(indices[1])]
